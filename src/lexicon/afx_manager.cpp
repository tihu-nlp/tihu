...
inline int CAfxManager::isRevSubset(const char *s1, const char *end_of_s2,
                                    int len) {
...
void CAfxManager::AffixCheck(const char *text, int len, const FLAG needflag,
                             CWordPtr &word) {
...
void CAfxManager::PrefixCheck(const char *text, int len, const FLAG needflag,
                              CWordPtr &word) {
...
void CAfxManager::SuffixCheck(const char *text, int len, CPfxEntry *pfx,
...
void CAfxManager::ParsEntry(struct hentry *he, CPfxEntry *pfx, CSfxEntry *sfx,
                            CWordPtr &word) {
  std::string text = he->data;
  std::string pos = he->data + he->word_len + 1;
  std::string pron = he->data + he->word_len + he->pos_len + 2;
  std::string lemma = he->data + he->word_len + he->pos_len + +he->pron_len + 3;
  std::string stem = text;
  int weight = he->weight;
  int lemma_weight = HashManager->GetLemmaWeight(lemma);
  if (lemma_weight > 0) {
    weight = lemma_weight;
  }
  ...
  if (sfx) {
    if (sfx->POS != ".") {
      if (sfx->POS[0] == '*') {
        std::string attr = sfx->POS.substr(1);
