...
CWord::CWord()
    : Type(TIHU_TOKEN_TYPE::UNKNOWN), Frequency(0), Length(0), Offset(0),
      Flags(0) {}

CWord::~CWord() {}

void CWord::SetNormed(const std::string &normed) {
    Normed = normed; //
}
...
std::string CWord::GetNormed() const {
    return Normed; //
}
