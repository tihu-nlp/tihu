...
int mbrowrap::start_mbrola(const char *voice_path) {
  ...
  if (mbr_pid == 0) {
    ...
    if (dup2(p_stdin[0], 0) == -1 || dup2(p_stdout[1], 1) == -1 ||
        dup2(p_stderr[1], 2) == -1) {
      snprintf(mbr_errorbuf, sizeof(mbr_errorbuf), "dup2(): %s\n",
               strerror(errno));
    ...
    execlp("./mbrola", "./mbrola", "-e", "-v", charbuf, voice_path, "-",
           "-.wav", (char *)NULL);
    /* if execution reaches this point then the exec() failed */
    snprintf(mbr_errorbuf, sizeof(mbr_errorbuf), "mbrola: %s\n",
             strerror(errno));
...
int mbrowrap::mbrola_died(void) {
  ...
  if (!pid) {
    ...
    if (WIFSIGNALED(status)) {
      ...
      snprintf(msgbuf, sizeof(msgbuf), "mbrola died by signal %d", sig);
      msg = msgbuf;
    } else if (WIFEXITED(status)) {
      int exst = WEXITSTATUS(status);
      snprintf(msgbuf, sizeof(msgbuf), "mbrola exited with status %d", exst);
      msg = msgbuf;
    } else {
  ...
  if (!len) {
    ...
    snprintf(mbr_errorbuf + len, sizeof(mbr_errorbuf) - len, ", (%s)", msg);
  return -1;
}
...
int mbrowrap::mbrola_has_errors(void) {
  ...
  for (;;) {
    result =
        read(mbr_error_fd, buf_ptr, sizeof(buffer) - (buf_ptr - buffer) - 1);
    if (result == -1) {
      if (errno == EAGAIN) {
        return 0;
      }
    ...
    for (; (lf = strchr(buf_ptr, '\n')); buf_ptr = lf + 1) {
      ...
      *lf = 0;
      ...
      if (lf == &buf_ptr[result - 1]) {
        log(mbr_errorbuf, sizeof(mbr_errorbuf), "%s", buf_ptr);
...
int mbrowrap::init_MBR(const char *voice_path) {
  ...
  mbr_samplerate =
      wavhdr[24] + (wavhdr[25] << 8) + (wavhdr[26] << 16) + (wavhdr[27] << 24);
...
int mbrowrap::flush_MBR(void) { return send_to_mbrola("\n#\n") == 3; }

int mbrowrap::getFreq_MBR(void) const { return mbr_samplerate; }
...
void mbrowrap::resetError_MBR(void) { mbr_errorbuf[0] = 0; }
