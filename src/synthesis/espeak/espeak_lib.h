...
typedef void (*ESPEAK_PROC_SETSYNTHCALLBACK)(t_espeak_callback* SynthCallback);
typedef void (*ESPEAK_PROC_SETURICALLBACK)(int (*UriCallback)(int, const char*, const char*));
typedef espeak_ERROR(*ESPEAK_PROC_SYNTH)(const void* text, size_t size, unsigned int position, espeak_POSITION_TYPE position_type, unsigned int end_position, unsigned int flags, unsigned int* unique_identifier, void* user_data);
typedef espeak_ERROR(*ESPEAK_PROC_SYNTH_MARK)(const void* text, size_t size, const char* index_mark, unsigned int end_position,  unsigned int flags, unsigned int* unique_identifier, void* user_data);
typedef espeak_ERROR(*ESPEAK_PROC_KEY)(const char* key_name);
typedef espeak_ERROR(*ESPEAK_PROC_CHAR)(wchar_t character);
typedef espeak_ERROR(*ESPEAK_PROC_SETPARAMETER)(espeak_PARAMETER parameter, int value, int relative);
typedef int (*ESPEAK_PROC_GETPARAMETER)(espeak_PARAMETER parameter, int current);
typedef espeak_ERROR(*ESPEAK_PROC_SETPUNCTUATIONLIST)(const wchar_t* punctlist);
typedef void (*ESPEAK_PROC_SETPHONEMETRACE)(int value, FILE* stream);
typedef void (*ESPEAK_PROC_COMPILEDICTIONARY)(const char* path, FILE* log, int flags);
typedef const espeak_VOICE**    (*ESPEAK_PROC_LISTVOICES)(espeak_VOICE* voice_spec);
typedef espeak_ERROR(*ESPEAK_PROC_SETVOICEBYNAME)(const char* name);
typedef espeak_ERROR(*ESPEAK_PROC_SETVOICEBYPROPERTIES)(espeak_VOICE* voice_spec);
