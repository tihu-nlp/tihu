...
char16_t *u16sep(char16_t **stringp, const char16_t delim) {
  ...
  if (*mp != '\0') {
    ...
    if (delim) {
      ...
      // don't use isspace() here, the string can be in some random
      // charset that's way different than the locale's
...
std::u16string UTF32ToUTF16(const std::u32string &utf32line) {
  ...
  for (size_t i = 0; i < bytes.size(); i += 2) {
    result.push_back(
        static_cast<char16_t>(static_cast<unsigned char>(bytes[i]) * 256 +
...
bool IsVowelPhoneme(char phoneme) {
  if (phoneme == 'a' ||                   // a and a:
      phoneme == 'e' || phoneme == 'o' || // o and ow
      phoneme == 'u' || phoneme == 'i' || phoneme == 'A') {
    return true;
  }
...
std::string ConcatPronunciations(const std::string &pronunciation_1,
  ...
  char last_1 = GetLastChar(pronunciation_1);
  char first_2 = GetFirstChar(pronunciation_2);

  /// تنباکوم عموت چاقوش
  if (last_1 == 'u' && first_2 == 'a') {
    /// tanbAkuaS -> tanbAkuS
...
bool EndsWithDetached(const std::string &value) {
  return EndsWith(value, CHR_U8_ALEF) || EndsWith(value, CHR_U8_DAL) ||
         EndsWith(value, CHR_U8_ZAL) || EndsWith(value, CHR_U8_RE) ||
         EndsWith(value, CHR_U8_ZE) || EndsWith(value, CHR_U8_WAW) ||
...
bool Replace(std::string &str, const std::string &search,
             const std::string &replace) {
  auto n = str.find(search);
  if (n != std::string::npos) {
    str.replace(n, search.length(), replace);
    return true;
  }
  return false;
}
