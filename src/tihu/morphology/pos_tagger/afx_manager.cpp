...
void CAfxManager::ParsEntry(struct hentry* he, CPfxEntry* pfx, CSfxEntry* sfx, CWordPtr &text)
{
    ...
    entry->SetRoot(root);
    entry->SetDictation(dictation);
    entry->SetLabel(lable);
    entry->SetFrequency(frequency);
    entry->SetPronunciation(pronunce);
    entry->SetDictation(dictation);
