...
int CTokenizer::ParsEvents(const CWordPtr &word, const char16_t *text) {
  ...
  while (1) {
    ...
    std::string event_type = UTF16ToUTF8(std::u16string(start_tag + 1, colon));

    std::string event_value = UTF16ToUTF8(std::u16string(colon + 1, end_tag));
